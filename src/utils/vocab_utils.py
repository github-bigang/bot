...
#         self.vocab_size, self.word_dim = map(int, header.split())
    ...
    def fromText_pickle(self, vec_path, voc=None):
        self.word2id = {}
        self.id2word = {}
        import cPickle
        word_vecs = cPickle.load(open(vec_path, 'rb'))
        self.vocab_size = len(self.word2id)
        self.word_vecs = np.zeros((self.vocab_size+1, self.word_dim), dtype=np.float32) # the last dimension is all zero
        for cur_index in xrange(self.vocab_size):
            self.word_vecs[cur_index] = word_vecs[cur_index]
...
#     print(vocab.word_dim)
    ...
    import sys
    word_vec_path = sys.argv[1] #'/u/zhigwang/zhigwang1/learn2rank/data/comp7.0/wordvec_crop.bin'
    out_path = sys.argv[2] #'/u/zhigwang/zhigwang1/learn2rank/models/vocab'
